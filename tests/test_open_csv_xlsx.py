...
def test_get_transactions_from_csv_ok(mock_read):
    mock_read.return_value.to_dict.return_value = [{'id': 650703.0,
                                  'state': 'EXECUTED',
    ...
    assert get_transactions_from_csv(mock_read) == [{'id': 650703.0,
                                  'state': 'EXECUTED',
                                  'date': '2023-09-05T11:30:32Z',
                                  'amount': 16210.0,
                                  'currency_name': 'Sol',
...
def test_get_transactions_from_csv_empty_file(capsys):
    ...
    out = capsys.readouterr().out
    assert out == f'Ошибка чтения CSV. Проверьте разделитель или формат данных.\n'
    mock_read.assert_called_once()
...
def test_get_transactions_from_excel_ok(mock_read):
    mock_read.return_value.to_dict.return_value = [{'id': 650703.0,
                                  ...
                                  'to': 'Счет 39745660563456619397',
                                  'description': 'Перевод организации'}]
    assert get_transactions_from_excel(mock_read) == [{'id': 650703.0,
                                  'state': 'EXECUTED',
                                  'date': '2023-09-05T11:30:32Z',
                                  'amount': 16210.0,
                                  'currency_name': 'Sol',
